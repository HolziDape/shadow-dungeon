var GLOW_SCALE = 0.4;  // global canvas glow damping
var DECO_SCALE = 0.45; // damping for orbital HUD decoration (rings, orbits,
                       // sweep lines, drifting particles). 1 = original look.
...
function _glowStroke(ctx, fn, color, lw, hBlur, tBlur) {
    ...
    ctx.lineWidth=lw+1.8; ctx.shadowBlur=GLOW_SCALE*(hBlur); ctx.globalAlpha=saved*0.5;
    fn(); ctx.stroke();
    ctx.lineWidth=lw; ctx.shadowBlur=GLOW_SCALE*(tBlur); ctx.globalAlpha=saved;
    fn(); ctx.stroke();
}
...
stock: {
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        ...
        ctx.strokeStyle=style.pulse; ctx.lineWidth=0.8; ctx.globalAlpha=DECO_SCALE*(ia*(0.28+spd*0.18));
        ctx.shadowBlur=GLOW_SCALE*(10); ctx.shadowColor=style.pulse; ctx.setLineDash([4,3]);
        ctx.beginPath();
        for(let i=0;i<6;i++){
        ...
        ctx.globalAlpha=DECO_SCALE*(ia*(0.14+spd*0.14)); ctx.strokeStyle=style.core; ctx.lineWidth=1.2;
        ctx.shadowBlur=GLOW_SCALE*(14); ctx.shadowColor=style.core;
        ctx.beginPath(); ctx.moveTo(0,0);
        ctx.lineTo(Math.cos(scanA)*42*sc, Math.sin(scanA)*42*sc); ctx.stroke();
        ctx.restore();

        // Dashed orbit ring — slowly rotating
        ctx.save(); ctx.strokeStyle=style.pulse; ctx.lineWidth=0.6; ctx.globalAlpha=DECO_SCALE*(ia*0.28);
        ...
        _radFill(ctx,0,-10*sc,36*sc,'rgba(200,255,255,0.55)','rgba(96,164,196,0.04)',ia); ctx.restore();
        ctx.save(); ctx.globalAlpha=ia;
        _glowStroke(ctx,()=>_shipPath(ctx,pts),style.core,1.6,22,8);
        ctx.shadowBlur=GLOW_SCALE*(4); ctx.shadowColor='#fff'; ctx.strokeStyle='rgba(255,255,255,0.85)';
        ctx.lineWidth=0.9; _shipPath(ctx,pts); ctx.stroke(); ctx.restore();

        // Panel lines — spine sways, cross-bars slide laterally
        ctx.save(); ctx.strokeStyle='rgba(111,183,197,0.45)'; ctx.lineWidth=0.6;
        ctx.shadowBlur=GLOW_SCALE*(4); ctx.shadowColor=style.core; ctx.globalAlpha=ia*0.7; ctx.lineCap='round';
        ...
        [[-20*sc,22*sc,'#c05a63'],[20*sc,22*sc,'#6ec496']].forEach(([wx,wy,col],i)=>{
            const blink=0.5+Math.sin(t*blinkRate+i*Math.PI)*0.5;
            ctx.globalAlpha=ia*blink*(0.8+spd*0.2); ctx.fillStyle=col;
            ctx.shadowBlur=GLOW_SCALE*(9+spd*6); ctx.shadowColor=col;
        ...
        eg.addColorStop(0,style.core); eg.addColorStop(0.5,'rgba(104,175,196,0.45)'); eg.addColorStop(1,'rgba(96,164,196,0)');
        ctx.fillStyle=eg; ctx.shadowBlur=GLOW_SCALE*(14+fwd*10); ctx.shadowColor=style.core;
        ...
        if(spd>0.3){
            ...
            [[-20*sc,8*sc],[20*sc,8*sc]].forEach(([sx,sy],i)=>{
                ...
                ctx.fillStyle=style.core; ctx.shadowBlur=GLOW_SCALE*(8); ctx.shadowColor=style.core;
                ctx.beginPath(); ctx.arc(sx+mvx*4*sc, sy-mvy*3*sc, 1.5+spd,0,Math.PI*2); ctx.fill();
            }); ctx.restore();
        }

        // Core glow orb
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(18); ctx.shadowColor=style.core; ctx.globalAlpha=ia;
        const cg=ctx.createRadialGradient(0,-2*sc,0,0,-2*sc,7*sc);
        cg.addColorStop(0,'#fff'); cg.addColorStop(0.5,style.core); cg.addColorStop(1,'rgba(104,175,196,0)');
        ctx.fillStyle=cg; ctx.beginPath(); ctx.arc(0,-2*sc,7*sc,0,Math.PI*2); ctx.fill(); ctx.restore();

        // Engine bar
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(8); ctx.shadowColor=style.trail; ctx.strokeStyle=style.trail;
...
ember_blade: {
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        ...
        [[20,54,style.pulse,0.32,18],[14,46,style.pulse,0.55,14],[9,40,style.core,0.75,10],[5,33,'#fffbe8',0.9,6]].forEach(([hw,h,col,a,blur])=>{
            ...
            ctx.fillStyle=col; ctx.shadowBlur=GLOW_SCALE*(blur+(fwd*6)); ctx.shadowColor=col; ctx.globalAlpha=ia*a*Math.min(1.0,fi);
        ...
        [-22,22].forEach((wx,si)=>{
            ...
            dg.addColorStop(0,style.pulse); dg.addColorStop(1,'rgba(201,122,53,0)');
            ctx.strokeStyle=dg; ctx.lineWidth=2.2+fwd*1.2;
            ctx.shadowBlur=GLOW_SCALE*(8+fwd*6); ctx.shadowColor=style.pulse; ctx.globalAlpha=ia*(0.7+fwd*0.2);
            ctx.beginPath(); ctx.moveTo(wx*sc,26*sc);
            ctx.quadraticCurveTo(cx, cy, wx*sc+sway, dripLen*sc); ctx.stroke();
            ctx.fillStyle=style.core; ctx.shadowBlur=GLOW_SCALE*(6); ctx.globalAlpha=ia*0.9;
            ctx.beginPath(); ctx.arc(wx*sc+sway,dripLen*sc,2.2+fwd,0,Math.PI*2); ctx.fill();
        }); ctx.restore();

        // Hull fill + glow
        ctx.save(); _shipPath(ctx,pts);
        _radFill(ctx,0,-20*sc,40*sc,'rgba(224,197,132,0.6)','rgba(140,72,48,0.1)',ia); ctx.restore();
        ctx.save(); ctx.globalAlpha=ia;
        ...
        ctx.save(); ctx.lineCap='round'; ctx.shadowColor=style.core; ctx.shadowBlur=GLOW_SCALE*(6+spd*8);
        [
            [[-2,-24],[2,-14],[-4,-4],[0,8]],
            [[4,-18],[1,-6],[4,4]],
            [[-5,-10],[-8,0],[-6,10]],
        ...
        ctx.shadowBlur=GLOW_SCALE*(8+cg2*5); ctx.lineWidth=1.2; ctx.globalAlpha=ia*(0.6+cg2*0.25);
        ctx.beginPath(); ctx.moveTo(-3*sc,-26*sc); ctx.lineTo(1*sc,-8*sc); ctx.lineTo(-4*sc,4*sc); ctx.lineTo(0,14*sc); ctx.stroke();
        ctx.restore();

        // Core glow
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(16+spd*8); ctx.shadowColor=style.core; ctx.globalAlpha=ia;
        const og=ctx.createRadialGradient(0,-4*sc,0,0,-4*sc,(8+spd*3)*sc);
        og.addColorStop(0,'#fff'); og.addColorStop(0.4,style.core); og.addColorStop(1,'rgba(201,122,53,0)');
        ...
        [[-26,-10],[24,-6],[-30,8],[28,10],[-18,4],[20,0],[-14,-18],[16,-14]].forEach(([ex,ey],i)=>{
            ...
            ctx.globalAlpha=DECO_SCALE*(ia*(0.45+Math.sin(t*3+i)*0.35));
            ctx.fillStyle=i%3===0?'#fffbe8':i%3===1?style.core:style.pulse;
            ctx.shadowBlur=GLOW_SCALE*(5+spd*5); ctx.shadowColor=style.core;
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        ...
        [-1,1].forEach(sign=>{
            ...
            [[36,10,0.6],[44,8,0.45],[52,6,0.28],[62,4,0.15]].forEach(([dist,h,a],li)=>{
                ...
                ctx.shadowBlur=GLOW_SCALE*(9-li*2+spd*4); ctx.shadowColor=style.pulse;
        ...
        [[24,0.9,0.50],[30,0.7,0.38],[36,0.5,0.27],[42,0.35,0.18],[50,0.2,0.10]].forEach(([rad,lw,ba],ri)=>{
            ...
            ctx.shadowBlur=GLOW_SCALE*(ri===0?12+spd*6:4); ctx.shadowColor=style.pulse;
        ...
        // Hull fill + glow
        ctx.save(); _shipPath(ctx,pts);
        _radFill(ctx,0,-14*sc,40*sc,'rgba(177,149,214,0.5)','rgba(72,56,102,0.06)',ia); ctx.restore();
        ctx.save(); ctx.globalAlpha=ia;
        ...
        ctx.strokeStyle='#cfb2dd'; ctx.lineWidth=1.1+spd*0.6;
        ctx.shadowBlur=GLOW_SCALE*(12+spd*10); ctx.shadowColor=style.core;
        [-1,1].forEach(sign=>{
            const jag=spd*4;
        ...
        ctx.fillStyle=style.pulse; ctx.shadowBlur=GLOW_SCALE*(10+spd*8); ctx.shadowColor=style.pulse; ctx.globalAlpha=ia*Math.min(0.7,ip);
        const vm=[
        ...
        // Core glow
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(20+spd*8); ctx.shadowColor=style.core; ctx.globalAlpha=ia;
        const vg=ctx.createRadialGradient(0,-6*sc,0,0,-6*sc,(9+spd*3)*sc);
        vg.addColorStop(0,'#fff'); vg.addColorStop(0.4,style.core); vg.addColorStop(1,'rgba(110,80,150,0)');
        ...
        ctx.save();
        const vColors=['#bb9bd8','#a184c9','#cfb2dd','#8800cc','#fff','#ff80ff','#aa44ff','#7700bb'];
        vColors.forEach((col,i)=>{
            const a=t*(0.8+i*0.15)+i*(Math.PI/4);
            const rad=(r+5+i%3*5)*(1+spd*0.3);
            ctx.globalAlpha=DECO_SCALE*(ia*(0.5+Math.sin(t*2.5+i)*0.35));
            ctx.fillStyle=col; ctx.shadowBlur=GLOW_SCALE*(6+spd*4); ctx.shadowColor=col;
        ...
        [[-28,-14],[26,-8],[-32,6],[30,10],[-24,20],[24,18]].forEach(([dx,dy],i)=>{
            ...
            ctx.fillStyle=i%2===0?'#fff':'#bb9bd8'; ctx.shadowBlur=GLOW_SCALE*(3+spd*4);
...
solar_flare: {
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        ...
        for(let i=0;i<24;i++){
            ...
            ctx.shadowBlur=GLOW_SCALE*([14,7,3][tier]+spd*3);
        ...
        ctx.save(); ctx.strokeStyle='#cf9440'; ctx.lineWidth=1.8+spd*1.0; ctx.lineCap='round';
        ctx.shadowBlur=GLOW_SCALE*(12+spd*8); ctx.shadowColor='#cf9440';
        ...
        ctx.globalAlpha=DECO_SCALE*(ia*(0.28+Math.abs(Math.sin(t*1.5))*0.2));
        ctx.setLineDash([3,3]); ctx.shadowBlur=GLOW_SCALE*(12); ctx.shadowColor=style.core;
        ctx.rotate(t*0.18);
        ...
        sg.addColorStop(0.75,'#cf9440'); sg.addColorStop(1,style.pulse);
        ctx.fillStyle=sg; ctx.beginPath(); ctx.arc(0,0,22*sc,0,Math.PI*2); ctx.fill();
        ctx.strokeStyle=style.core; ctx.lineWidth=1.4; ctx.shadowBlur=GLOW_SCALE*(8);
        ...
        [[-6,-4,3,0.28],[6,4,2.4,0.21],[2,-8,1.6,0.35],[-3,7,2.0,0.18]].forEach(([sx,sy,sr,spd2],i)=>{
            ...
            ctx.globalAlpha=DECO_SCALE*(ia*0.48); ctx.fillStyle='#9c7030'; ctx.shadowBlur=GLOW_SCALE*(0);
            ctx.beginPath(); ctx.arc((sx+ox)*sc,(sy+oy)*sc,sr*sc,0,Math.PI*2); ctx.fill();
        }); ctx.restore();

        // Inner hot-spot
        ctx.save(); ctx.globalAlpha=ia; ctx.shadowBlur=GLOW_SCALE*(0);
        ...
        for(let i=0;i<6;i++){
            ...
            ctx.globalAlpha=DECO_SCALE*(ia*tw); ctx.fillStyle=i%2===0?style.ship:style.core;
            ctx.shadowBlur=GLOW_SCALE*(8+spd*4); ctx.shadowColor=style.core;
        ...
        for(let i=0;i<10;i++){
            ...
            ctx.fillStyle='rgba(227,207,154,0.85)'; ctx.shadowBlur=GLOW_SCALE*(2);
...
crimson_afterburn: {
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        ...
        for(let i=0;i<8;i++){
            ...
            ctx.lineWidth=0.7+i%2*0.4+spd*0.5; ctx.shadowBlur=GLOW_SCALE*(4+spd*6); ctx.shadowColor=style.pulse;
        ...
        [-1,1].forEach(sign=>{
            [[22,52,style.pulse,0.32,18],[14,44,style.pulse,0.58,12],[8,38,'#fffbe8',0.88,8]].forEach(([hw,h,col,a,blur])=>{
                ...
                ctx.fillStyle=col; ctx.shadowBlur=GLOW_SCALE*(blur+fwd*12); ctx.shadowColor=col; ctx.globalAlpha=ia*Math.min(0.95,a*fi*0.9);
                ctx.beginPath();
        ...
        cf.addColorStop(0,style.core); cf.addColorStop(1,'rgba(200,90,95,0)');
        ...
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(10+spd*6); ctx.shadowColor=style.pulse;
        [-1,1].forEach(sign=>{
        ...
        bg.addColorStop(0,'rgba(255,255,255,0.75)'); bg.addColorStop(0.3,style.ship); bg.addColorStop(1,'rgba(120,52,58,0.3)');
        ...
        ctx.shadowBlur=GLOW_SCALE*(12+spd*6); ctx.shadowColor=style.core; ctx.lineCap='round'; ctx.globalAlpha=ia*0.75;
        ...
        [14,22].forEach((arcR,si)=>{
            ...
            ctx.lineWidth=1.2-si*0.3+spd*0.5; ctx.shadowBlur=GLOW_SCALE*(8+spd*6); ctx.shadowColor=style.pulse;
        ...
        // Core glow
        ctx.save(); ctx.shadowBlur=GLOW_SCALE*(18+spd*8); ctx.shadowColor=style.core; ctx.globalAlpha=ia;
        const cg=ctx.createRadialGradient(0,0,0,0,0,(7+spd*3)*sc);
        cg.addColorStop(0,'#fff'); cg.addColorStop(0.4,style.core); cg.addColorStop(1,'rgba(208,113,111,0)');
        ...
        for(let i=0;i<6;i++){
            ...
            ctx.globalAlpha=DECO_SCALE*(ia*tw);
            ctx.fillStyle=i%3===0?style.core:i%3===1?style.pulse:'rgba(255,200,200,0.8)';
            ctx.shadowBlur=GLOW_SCALE*(8+spd*4); ctx.shadowColor=style.pulse;
            const pr=(r+9+spd*8);
        ...
        ctx.shadowBlur=GLOW_SCALE*(10+spd*8); ctx.shadowColor=style.pulse;
...
aurora_zero: {
    ...
    drawBody(ctx, r, style, rv, ia, mvx=0, mvy=0) {
        const sc=r/38, t=_renderNow * 0.001;
        const spd=Math.hypot(mvx,mvy);
        const ROW=['#97c7d6','#fffbe8','#d6b36a','#d49aa4','#a184c9'];
        ...
        [[-8,0.70,3.0],[ 4,0.52,2.0],[-4,0.38,1.4],[12,0.26,1.0],[-12,0.16,0.7]].forEach(([yoff,a,lw],ri)=>{
            ...
            ctx.shadowBlur=GLOW_SCALE*(lw*10); ctx.shadowColor=style.pulse;
        ...
        ctx.save();
        [[38,'#97c7d6',[5,3],1.4],[44,'#d6b36a',[2,4],0.8],[32,'#d49aa4',[3,5],1.0]].forEach(([rad,col,dash,lw],ri)=>{
            ctx.setLineDash(dash); ctx.lineWidth=lw; ctx.strokeStyle=col;
            ctx.shadowBlur=GLOW_SCALE*(ri===0?10:6); ctx.shadowColor=col;
            ctx.globalAlpha=DECO_SCALE*(ia*(0.48+Math.sin(t*1.2+ri*0.8)*0.15));
        ...
        fragBase.forEach((tri,ti)=>{
            ...
            ctx.shadowBlur=GLOW_SCALE*(8+spd*4); ctx.shadowColor=ROW[ti%ROW.length];
        ...
        // Hull fill
        ctx.save(); _shipPath(ctx,pts);
        _radFill(ctx,0,-20*sc,42*sc,'rgba(255,255,210,0.65)','rgba(125,167,196,0.06)',ia); ctx.restore();

        // Hull rainbow stroke
        ctx.save(); ctx.globalAlpha=ia; ctx.shadowBlur=GLOW_SCALE*(28+spd*6); ctx.shadowColor=style.pulse;
        ctx.strokeStyle=makeRG(-24*sc,0,24*sc,0); ctx.lineWidth=2.2;
        _shipPath(ctx,pts); ctx.stroke();
        ctx.shadowBlur=GLOW_SCALE*(6); ctx.shadowColor='#fff'; ctx.strokeStyle='rgba(255,255,255,0.55)'; ctx.lineWidth=0.9;
        ...
        // Muzzle flash — fires at the nose tip, scales with firepower, capped at 4×base
        {
            ...
            if (flash > 0.01) {
                ...
                ctx.shadowBlur=GLOW_SCALE*(glow); ctx.shadowColor = '#fff';
                ctx.beginPath(); ctx.moveTo(0, noseY); ctx.lineTo(0, noseY - spikeLen); ctx.stroke();
                // colour overlay on spike
                ctx.strokeStyle = '#97c7d6'; ctx.lineWidth = 1.0 + fp * 0.8;
                ...
                ctx.shadowColor = '#fff'; ctx.shadowBlur=GLOW_SCALE*(8);
                ...
                ctx.shadowBlur=GLOW_SCALE*(glow * 0.6); ctx.shadowColor = '#d6b36a';
                ...
                ROW.forEach((col, i) => {
                    ...
                    ctx.shadowBlur=GLOW_SCALE*(10); ctx.shadowColor = col;
        ...
        ctx.shadowBlur=GLOW_SCALE*(8); ctx.shadowColor=style.pulse;
        ...
        ctx.save();
        ctx.shadowBlur=GLOW_SCALE*(24+spd*6); ctx.shadowColor=style.core; ctx.globalAlpha=ia*0.55;
        ctx.fillStyle=style.pulse; ctx.beginPath(); ctx.arc(0,-2*sc,11*sc,0,Math.PI*2); ctx.fill();
        ctx.shadowBlur=GLOW_SCALE*(16); ctx.globalAlpha=ia; ctx.fillStyle=style.core;
        ctx.beginPath(); ctx.arc(0,-2*sc,6*sc,0,Math.PI*2); ctx.fill();
        ctx.shadowBlur=GLOW_SCALE*(0); ctx.fillStyle='#fff';
        ...
        const drawStar4=(cx,cy,sz,rot,col,alpha)=>{
            ...
            ctx.shadowBlur=GLOW_SCALE*(12); ctx.shadowColor=col;
            ...
            ctx.globalAlpha=alpha; ctx.fillStyle='#fff'; ctx.shadowBlur=GLOW_SCALE*(6);
        ...
        const drawDiamond=(cx,cy,w,h,rot,col,alpha)=>{
            ctx.save(); ctx.translate(cx,cy); ctx.rotate(rot);
            ctx.globalAlpha=DECO_SCALE*(alpha); ctx.shadowBlur=GLOW_SCALE*(14+spd*6); ctx.shadowColor=col;
            ctx.fillStyle=col;
            ...
            ctx.shadowBlur=GLOW_SCALE*(6); ctx.shadowColor='#fff';
        ...
        [[34,3.0,0.9],[42,2.0,0.6]].forEach(([yd,lw,a])=>{
            ...
            ctx.shadowBlur=GLOW_SCALE*(10); ctx.shadowColor=style.pulse; ctx.globalAlpha=ia*a;
